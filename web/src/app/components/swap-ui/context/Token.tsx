...
export function TokenContextProvider(props: any) {
  ...
  useEffect(() => {
    ...
    // Fetch SOL balance.
    const pubkey = new PublicKey(provider.wallet.publicKey);
    provider.connection
...
export function useOwnedTokenAccount(
  mint?: PublicKey
): { publicKey: PublicKey; account: any } | null | undefined {
...
export function useMint(mint?: PublicKey): any | undefined | null {
  const { provider } = useTokenContext();
  ...
  const asyncMintInfo = useAsync(async () => {
    ...
    const mintInfo = getMint(provider.connection, mint);
...
export function setMintCache(pk: PublicKey, account: any) {
