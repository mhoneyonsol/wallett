...
interface PopularTokensByEndpoint {
  [endpoint: string]: PopularToken[];
}
...
const POPULAR_TOKENS: PopularTokensByEndpoint = {
  [MAINNET_URL]: [
    {
...
const TokenListContext = createContext<TokenListContextValue | undefined>(
  undefined
);

export function useTokenInfos(): TokenInfo[] | null {
  const context = useContext(TokenListContext);
  if (!context) {
    throw new Error(
      'useTokenInfos must be used within a TokenRegistryProvider'
    );
  }
  return context.tokenInfos;
}
...
export const TokenRegistryProvider: React.FC<TokenRegistryProviderProps> = (
  ...
  const [tokenInfos, setTokenInfos] = useState<TokenInfo[] | null>(null);
    ...
    tokenListProvider.resolve().then((tokenListContainer) => {
      ...
      if (cluster?.clusterSlug) {
        const filteredTokenListContainer =
          tokenListContainer.filterByClusterSlug(cluster.clusterSlug);
        const tokenInfos =
...
export function useTokenInfo(mint: PublicKey | null) {
  const { endpoint } = useConnectionConfig();
...
export function useUpdateTokenName() {
  const { endpoint } = useConnectionConfig();
  return useCallback(
    (mint: PublicKey, name: string, symbol: string) => {
      if (!name || !symbol) {
        if (name) {
          symbol = name;
        } else if (symbol) {
...
export function usePopularTokens() {
  const tokenInfos = useTokenInfos();
  const { endpoint } = useConnectionConfig();
  return (POPULAR_TOKENS[endpoint] ?? []).map((tok: PopularToken) =>
